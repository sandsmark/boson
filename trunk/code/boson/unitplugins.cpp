 ...
 if (mProductionState <= productionTime) {
	if (mProductionState == productionTime) {
		...
		if (speciesTheme()->unitProperties(id)->isFacility()) {
			game()->slotAddChatSystemMessage(i18n("A %1 has been produced - place it on the map to start construction!").arg(prop->name()), player());
			return;
		} else {
			game()->slotAddChatSystemMessage(i18n("A %1 has been produced and will be placed on the map now").arg(prop->name()), player());
		}
		...
		game()->slotAddChatSystemMessage(i18n("%1 could not be placed on the map - no free cell found. Place it manuall!").arg(prop->name()), player());
	} else {
		mProductionState = mProductionState + 1;
	}
 }
