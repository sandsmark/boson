...
//	reinitGame();
 ...
 if (!playField.preLoadPlayField(fileName)) {
	boError() << k_funcinfo << "preloading failed" << endl;
	return;
 }
 QMap<QString, QByteArray> files;
 if (!playField.loadFromDiskToFiles(files)) {
	boError() << k_funcinfo << "loading failed" << endl;
	return;
 }
 if (!files.contains("players.xml")) {
	boError() << k_funcinfo << "did not find players.xml" << endl;
	return;
 }

 // AB: we should update our widget - add players (human/computer) until
 ...
 if (boGame->playerCount() != 0) {
	boError() << k_funcinfo << "no player are allowed at this point" << endl;
	return;
 }
 QDomNodeList list = playersRoot.elementsByTagName("Player");
 if (list.count() == 0) {
	boError() << k_funcinfo << "no players in savegame" << endl;
	return;
 }
 bool ok = false;
 unsigned int localId = playersRoot.attribute("LocalPlayerId").toUInt(&ok);
 if (!ok) {
	boError() << k_funcinfo << "no LocalPlayerId" << endl;
	return;
 }
 for (unsigned int i = 0; i < list.count(); i++) {
	...
	if (!ok) {
		boError() << k_funcinfo << "invalid teamcolor" << endl;
		return;
	}
	if (species.isEmpty()) {
		boError() << k_funcinfo << "invalid SpeciesTheme" << endl;
		// TODO: check whether species actually exists and can get
		// loaded
		return;
	}
	if (boGame->findPlayer(id)) {
		boError() << k_funcinfo << "id " << id << " already in the game" << endl;
		return;
	}
