...
# AB: required for kde3uic
IF(EXISTS "${CMAKE_ROOT}/Modules/kde3init_dummy.cpp.in")
   SET(KDE3_MODULE_DIR "${CMAKE_ROOT}/Modules")
ELSE(EXISTS "${CMAKE_ROOT}/Modules/kde3init_dummy.cpp.in")
   SET(KDE3_MODULE_DIR "${CMAKE_SOURCE_DIR}/cmake/modules")
ENDIF(EXISTS "${CMAKE_ROOT}/Modules/kde3init_dummy.cpp.in")
...
MACRO(KDE3_AUTOMOC)
   ...
   FOREACH (_current_FILE ${ARGN})
      ...
      IF (EXISTS ${_tmp_FILE})
         ...
         # AB: if "SKIP_AUTOMOC" is set to true, we will not handle this file
         #     here. this is required to make bouic work correctly:
         #     we need to add generated .cpp files to the sources (to compile
         #     them), but we cannot let automoc handle them, as the .cpp files
         #     don't exist yet when cmake is run for the very first time on them
         #     -> however the .cpp files might exist at a later run. at that
         #        time we need to skip them, so that we don't add two different
         #        rules for the same moc file
         GET_SOURCE_FILE_PROPERTY(_skip ${_tmp_FILE} SKIP_AUTOMOC)

         STRING(REGEX MATCHALL "#include +[^ ]+\\.moc[\">]" _match "${_contents}")
         IF(_match AND NOT _skip)
...
#               SET(_header ${CMAKE_CURRENT_SOURCE_DIR}/${_basename}.h)
               ...
               # AB: bouic includes .moc and uses absolute filenames in sources
               #     uic does not suffer from this problem, as it does not
               #     include .moc files
               STRING(REGEX MATCH "^/" _isAbsolute "${_current_FILE}")
               IF(_isAbsolute)
                  KDE3_ADD_FILE_DEPENDANCY(${_current_FILE} ${_moc})
               ELSE(_isAbsolute)
                  KDE3_ADD_FILE_DEPENDANCY(${CMAKE_CURRENT_SOURCE_DIR}/${_current_FILE} ${_moc})
               ENDIF(_isAbsolute)

            ENDFOREACH (_current_MOC_INC)
         ENDIF(_match AND NOT _skip)
