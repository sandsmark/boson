...
<pre>$ cvs -z3 -d:pserver:anonymous@cvs.boson.sourceforge.net:/cvsroot/boson co -P code<br>$ cvs -z3 -d:pserver:anonymous@cvs.boson.sourceforge.net:/cvsroot/boson co -P data</pre>
Now you need to link the admin dir from the code directory:
<pre>$ cd data && ln -s ../code/admin admin</pre>
Since these files are from CVS, you need to create the makefiles in both the code and data directory first by doing
<pre>$ make -f Makefile.cvs</pre> Finally, you can configure, compile and install boson:
<pre>$ ./configure<br>$ make<br># make install</pre>
When configuring fails, check whether you have all the <a href='info.php'>software dependencies</a> installed.
<br>To stay up to date, you can update both code and data directories later on by executing
