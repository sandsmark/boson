...
void KMessageSocket::processNewData ()
{
  ...
  while (mSocket->bytesAvailable() > 0)
  {
    if (mAwaitingHeader)
    {
      ...
      if (v != 'M')
      {
        boWarning(11001) << k_funcinfo << ": Received unexpected data, magic number wrong!" << endl;
        continue;
      }
  ...
  : KMessageIO (parent, name), mPartner (0)
{
  ...
  {
    boWarning(11001) << k_funcinfo << ": Object is already connected!" << endl;
    return;
  }
...
void KMessageDirect::send (const QByteArray &msg)
{
  if (mPartner)
    ...
    boError(11001) << k_funcinfo << ": Not yet connected!" << endl;
}
...
{
  boDebug(11001) << "@@@KMessageProcess::Delete process" << endl;
  if (mProcess)
  {
    mProcess->kill();
    delete mProcess;
...
{
  // Start process
  boDebug(11001) << "@@@KMessageProcess::Start process" << endl;
  mProcessName=file;
  mProcess=new KProcess;
  int id=0;
  *mProcess << mProcessName << QString("%1").arg(id);
  boDebug(11001) << "@@@KMessageProcess::Init:Id= " << id << endl;
  boDebug(11001) << "@@@KMessgeProcess::Init:Processname: " << mProcessName << endl;
...
void KMessageProcess::writeToProcess()
{
  ...
  //  boDebug(11001) << " @@@@@@ writeToProcess::SEND to process " << mSendBuffer->size() << " BYTE " << endl;
...
void KMessageProcess::slotReceivedStderr(KProcess * proc, char *buffer, int buflen)
{
  ...
  char *p;
  char *pos;
//  boDebug(11001)<<"############# Got stderr " << buflen << " bytes" << endl;
...
{
  boDebug(11001) << "$$$$$$ " << k_funcinfo << ": Received " << buflen << " bytes over inter process communication" << endl;
  ...
  while (mReceiveCount>2*sizeof(long))
  {
    ...
    if (*p1!=0x4242aeae)
    {
      boDebug(11001) << k_funcinfo << ": Cookie error...transmission failure...serious problem..." << endl;
//      for (int i=0;i<mReceiveCount;i++) fprintf(stderr,"%02x ",mReceiveBuffer[i]);fprintf(stderr,"\n");
    }
    len=(int)(*p2);
    if (len<2*sizeof(long))
    {
      boDebug(11001) << k_funcinfo << ": Message size error" << endl;
      break;
    }
    if (len<=mReceiveCount)
    {
      boDebug(11001) << k_funcinfo << ": Got message with len " << len << endl;
...
{
  boDebug(11001) << "Process exited (slot)" << endl;
