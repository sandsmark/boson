...
public:
	...
	BosonMiniMapDisplay* mMiniMapDisplay;
	BoTexture* mLogoTexture;
};

BosonMiniMap::BosonMiniMap(QWidget* parent)
	: QWidget(parent)
{
 ...
 d->mMiniMapDisplay = new BosonMiniMapDisplay(this);
 layout->addWidget(d->mMiniMapDisplay);
...
#if 1
 QPushButton* zoomIn = new QPushButton(i18n("+"), this);
 connect(zoomIn, SIGNAL(signalClicked()),
		this, SLOT(slotZoomIn()));
 buttonsLayout->addWidget(zoomIn);
 QPushButton* zoomOut = new QPushButton(i18n("-"), this);
...
BosonGLMiniMapView* BosonMiniMap::miniMapView() const
{
 return d->mMiniMapDisplay->miniMapView();
}

void BosonMiniMap::quitGame()
{
 setLocalPlayerIO(0);
 d->mMiniMapDisplay->quitGame();
}

void BosonMiniMap::setLocalPlayerIO(PlayerIO* io)
{
 d->mMiniMapDisplay->setLocalPlayerIO(io);
}

bool BosonMiniMap::showMiniMap() const
{
 return d->mMiniMapDisplay->showMiniMap();
}

void BosonMiniMap::slotShowMiniMap(bool s)
{
...
void BosonMiniMap::setImageTheme(const QString& theme)
{
 d->mImageTheme = theme;
...
void BosonMiniMap::setZoomImages(const QImage& in_, const QImage& out_, const QImage& defaultZoom_)
{
 int w = in_.width();
 w = qMax(w, out_.width());
...
QImage BosonMiniMap::imageFromTheme(const QString& file, const QString& theme) const
{
...
void BosonMiniMap::createMap(BosonCanvas* c, const BoGLMatrices* gameGLMatrices)
{
 BO_CHECK_NULL_RET(c);
...
void BosonMiniMap::slotAdvance(unsigned int advanceCallsCount)
{
 if (miniMapView()) {
...
void BosonMiniMap::slotUpdateTerrainAtCorner(int x, int y)
{
 if (miniMapView()) {
...
void BosonMiniMap::slotExplored(int x, int y)
{
 if (miniMapView()) {
...
void BosonMiniMap::slotUnexplored(int x, int y)
{
 if (miniMapView()) {
...
void BosonMiniMap::initFogOfWar(PlayerIO* p)
{
 if (miniMapView()) {
...
unsigned int BosonMiniMap::mapWidth() const
{
 if (miniMapView()) {
...
unsigned int BosonMiniMap::mapHeight() const
{
 if (miniMapView()) {
