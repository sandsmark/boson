...
#ifndef BOQTSTARTUPWIDGET_H
#define BOQTSTARTUPWIDGET_H

#include <QEvent>
#include <QWidget>

class Player;
class BoLoadingWidget;
class KCmdLineArgs;
class BosonStartupNetwork;

class BoQtStartupWidgetPrivate;
class BoQtStartupWidget : public QWidget
{
	Q_OBJECT
public:
	enum WidgetId {
	...
	BoQtStartupWidget(QWidget* parent = 0);
	~BoQtStartupWidget();
...
protected:
	...
	BoLoadingWidget* loadingWidget() const;
...
protected slots:
	...
	void slotVisibleWidgetChanged(int index);

private:
	void init();

	int widgetId2Index(WidgetId id) const;

private:
	BoQtStartupWidgetPrivate* d;
};
