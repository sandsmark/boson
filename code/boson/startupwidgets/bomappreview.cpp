...
BoMapPreviewDisplay::BoMapPreviewDisplay(QWidget* parent)
	: QWidget(parent)
{
 d = new BoMapPreviewDisplayPrivate;
...
BoMapPreviewDisplay::~BoMapPreviewDisplay()
{
 delete d->mViewport;
 delete d;
}

QSize BoMapPreviewDisplay::sizeHint() const
{
 // FIXME: hardcoded
 return QSize(150, 150);
}

void BoMapPreviewDisplay::setPreview(const QImage& image)
{
 if (image.isNull()) {
...
void BoMapPreviewDisplay::slotWidgetResized()
{
 d->mViewport->setViewSize(width(), height());
}

void BoMapPreviewDisplay::slotMouseEvent(QMouseEvent* e)
{
 QPoint pos = e->pos();
...
void BoMapPreviewDisplay::slotWheelEvent(QWheelEvent* e)
{
 if (e->delta() > 0) {
	d->mViewport->zoomIn();
 } else {
...
public:
	...
	BoMapPreviewDisplay* mPreviewDisplay;
};


BoMapPreview::BoMapPreview(QWidget* parent)
    : QWidget(parent)
{
 d = new BoMapPreviewPrivate;
 ...
 d->mPreviewDisplay = new BoMapPreviewDisplay();
 layout->addWidget(d->mPreviewDisplay);
 layout->addStretch();
}

BoMapPreview::~BoMapPreview()
{
 delete d;
}

void BoMapPreview::setPlayField(const BPFPreview& preview)
{
 if (!preview.isLoaded()) {
