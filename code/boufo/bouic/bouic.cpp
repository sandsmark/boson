...
static bool writeDeclaration(Q3TextStream& header, const QDomElement& widgets, const QDomElement& methods, const QString& fileNameBase);
static bool writeImplementation(Q3TextStream& cpp, const QDomElement& widgets, const QDomElement& methods, const QString& fileNameBase);
static bool writeConstructorImplementation(Q3TextStream& cpp, const QDomElement& widgets, const QDomElement& methods, const QString& fileNameBase);
static bool writeCreateChildren(Q3TextStream& cpp, const QDomElement& e, const QString& parent);
static bool writePropertiesList(Q3TextStream& cpp, const QDomElement& root);
static bool writeConnections(Q3TextStream& cpp, const QDomElement& root);
...
int main(int argc, char** argv)
{
 ...
 if (!openFile(fileName, &doc)) {
	qCritical() << "Error reading" << fileName;
	return 1;
 }
...
bool openFile(const QString& fileName, QDomDocument* doc)
{
 ...
 if (!doc->setContent(&file)) {
	qCritical() << "Could not parse file" << fileName;
	return false;
 }
 file.close();
...
bool saveTo(const QString& _fileBase, QDomDocument& doc)
{
 ...
 if (fileBase.contains('/')) {
	fileBase = fileBase.right(fileBase.length() - fileBase.lastIndexOf('/') - 1);
 }
 if (!fileCpp.open(QIODevice::WriteOnly)) {
	qCritical().nospace() << "Could not open " << fileBase << ".cpp for writing";
	return false;
 }
 if (!fileH.open(QIODevice::WriteOnly)) {
	qCritical().nospace() << "Could not open " << fileBase << ".h for writing";
	return false;
 }
 Q3TextStream cpp(&fileCpp);
 Q3TextStream header(&fileH);
 QDomElement root = doc.documentElement();
 QDomElement widgets = root.namedItem("Widgets").toElement();
...
bool writeDeclaration(Q3TextStream& header, const QDomElement& widgets, const QDomElement& methods, const QString& fileNameBase)
{
 header << g_headline;

 header << "#ifndef " << fileNameBase.toUpper() << "_H\n";
 header << "#define " << fileNameBase.toUpper() << "_H\n";
 header << "\n";
...
bool writeImplementation(Q3TextStream& cpp, const QDomElement& widgets, const QDomElement& methods, const QString& fileNameBase)
{
 cpp << g_headline;
 ...
 cpp << "\n";
 for (int i = 0; i < g_addIncludes.count(); i++) {
...
bool writeConstructorImplementation(Q3TextStream& cpp, const QDomElement& widgets, const QDomElement& methods, const QString& fileNameBase)
{
...
bool writeCreateChildren(Q3TextStream& cpp, const QDomElement& root, const QString& parent)
{
...
bool writePropertiesList(Q3TextStream& cpp, const QDomElement& root)
{
...
bool writeConnections(Q3TextStream& cpp, const QDomElement& root)
{
 if (root.isNull()) {
