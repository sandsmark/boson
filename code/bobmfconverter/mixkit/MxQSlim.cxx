...
bool MxEdgeQSlim::decimate_until_error(float maxerror, uint minfaces)
{
    MxPairContraction local_conx;

    while( valid_faces > minfaces )
    {
	MxQSlimEdge *info = (MxQSlimEdge *)heap.extract();
	if( !info ) { return false; }
...
#include <stdio.h>
float MxEdgeQSlim::rdecimate(uint target, float& maxerror, float& minerror, float& totalerror, float& lasterror)
{
    MxPairContraction local_conx;
    float error = 0.0;
	...
	if( m->vertex_is_valid(v1) && m->vertex_is_valid(v2) )
	{
	    ...
	    totalerror += error;
	    printf("    Collapsing %d-%d, error: %.4f\n", v1, v2, error);
